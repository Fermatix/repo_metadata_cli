...
> **Примечание.** Метрики `commit_count`, `contributors`, `total_pr_count` и `reviewed_pr_count` будут равны 0 — git-история отсутствует. Все остальные метрики (LOC, языки, тесты, CI, документация и др.) рассчитываются в полном объёме.
...
| `commit_count` = 0, `contributors` = 0 | Ожидаемо для локальных директорий без git. Все остальные метрики рассчитываются корректно. |

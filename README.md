...
> **Примечание про jscpd.** Без работающего jscpd команда `repo-metadata metadata`
...
> дублей. Проверяется не только наличие в `PATH`, но и запуск `jscpd --version`:
> jscpd это npm-пакет с шебангом `#!/usr/bin/env node`, и без Node.js в `PATH`
> он находится, но падает с кодом 127. Если дупликация не нужна, добавьте флаг
> `--allow-missing-jscpd` — расчёт пойдёт с нулевыми колонками осознанно.
>
> Node.js должен быть виден в той же оболочке, где запускается расчёт. Если он
> ставился через nvm, тот подключается только в интерактивной оболочке, и из-под
> IDE, cron или systemd ноды не будет.
